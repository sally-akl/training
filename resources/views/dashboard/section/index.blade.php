...
<div class="card">
  ...
  <div class="card-body border-bottom py-3">
    ...
    <div class="card-footer d-flex align-items-center">
      {{$sections->links('dashboard.vendor.pagination.default')}}
    </div>
  </div>
</div>
@endif
...
<div class="modal modal-blur fade" id="add_edit_modal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form method="POST" action="{{ url('dashboard/sections') }}" class="form_submit_model">
...
<script type="text/javascript">
  ...
  $(".edit_btn").on("click",function()
  {
      ...
      var url_edit = '{{url("/dashboard/sections")}}'+"/"+id;
      $(".form_submit_model").attr("action",url_edit);
      $.ajax({
          url: '{{url("/dashboard/sections")}}'+"/"+id+"/edit",
          type: 'GET',
          dataType: 'json',
          success: function (response) {
            $("input[name='title']").val(response.section_name);
  ...
  $(".add_btn").on("click",function(){
      $('#add_edit_modal').modal('show');
      $("input[name='method_type']").val("add");
      $(".form_submit_model").attr("action",'{{url("/dashboard/sections")}}');
      return false;
  });
  ...
  $(".delete_it_sure").on("click",function(){
    ...
    var url_delete = '{{url("/dashboard/sections")}}'+"/"+id;
